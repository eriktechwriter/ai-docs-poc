---
slug: trend-vision-one-configure-teamed-nics
title: Configure teamed NICs
last_updated: '2025-09-08'
ai_context:
  source_file: configure-teamed-nics.md
  conversion_date: '2025-09-08T23:42:22.650Z'
...
NIC teaming with the agent requires Windows 2003 requires SP 2 or later.
:::

:::danger[Warning]

Don't add or remove network interfaces from a teamed NIC except immediately before running the installer. Otherwise network connectivity may fail or the computer may not be correctly detected by Server & Workload Protection. The agent's network driver is bound to network interfaces when you install or upgrade; the agent does not continuously monitor for changes after.
:::
...
On Solaris, if multiple interfaces are on the same subnet, the operating system may route packets through any of the interfaces. Because of this, Server & Workload Protection firewall stateful configuration options and IPS rules should be applied to all interfaces equally.
:::
::::::
