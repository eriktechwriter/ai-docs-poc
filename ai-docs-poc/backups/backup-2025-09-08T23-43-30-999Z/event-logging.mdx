---
slug: trend-vision-one-event-logging
title: About Server & Workload Protection event logging
last_updated: '2025-09-08'
ai_context:
  source_file: event-logging.md
  conversion_date: '2025-09-08T23:42:19.666Z'
conversion_status: converted
translation_ready: true
---

:::: {role="main"}
Agents record when a protection module rule or condition is triggered (a "security event"). Agents and Server & Workload Protection also records when administrative or system-related events occur (a "system event"), such as an administrator logging in, or agent software being upgraded. Event data is used to populate the various reports and graphs in Server & Workload Protection.

To view events, go to *Events & Reports* in Server & Workload Protection.

### Where are event logs on the agent? {#where-are-event-logs-on-the-agent}
...
These locations only contain standard-level logs; diagnostic debug-level logs have a different location. For performance reasons, debug-level logging is not enabled by default. You should only enable debug logging if diagnosing an issue with Trend Micro technical support, and make sure to disable debug logging when you are done. For more information, see [Enabling detailed logging on Deep Security Agent (DSA)](https://success.trendmicro.com/solution/1096332).
:::

### When are events sent to Server & Workload Protection? {#when-are-events-sent-to-server-workload-protection}

Most events that take place on a computer are sent to Server & Workload Protection during the next heartbeat operation except the following, which will be sent right away if communication settings allow relays/agents to initiate communication:
...
### How long are events stored? {#how-long-are-events-stored}

Server & Workload Protection retains security events for 4 weeks and system events for 13 weeks. Customers requiring a longer event retention period should consider exporting events to an external SIEM. For more information, see [Forward Server & Workload Protection events to an external syslog or SIEM server](forward-syslog-siem-server.mdx).
...
- [Application Control events](application-control-events.mdx)
- [Firewall events](firewall-events.mdx)
- [Integrity Monitoring events](integrity-monitoring-events.mdx)
- [Intrusion Prevention events](intrusion-prevention-events.mdx)
- [Log Inspection events](log-inspection-events.mdx)
- [Web Reputation events](web-reputation-events.mdx)
- [Device Control events](device-control-events.mdx)

The firewall stateful configuration in effect on a computer can be modified to enable or disable TCP, UDP, and ICMP event logging. To edit the properties of a stateful firewall configuration, go to **Policies → Common Objects → Other → Firewall Stateful Configurations**. The logging options are in the *TCP*, *UDP*, and *ICMP* tabs of the firewall stateful configuration's *Properties* window. For more information about firewall events, see [Firewall events](firewall-events.mdx).

### See the events associated with a policy or computer {#see-the-events-associated-with-a-policy-or-computer}

The Policy editor and theComputer editor both have *Events* tabs for each protection module. The policy editor displays events associated with the current policy. The computer editor displays events specific to the current computer.

### View details about an event {#view-details-about-an-event}
...
The *Tags* tab displays tags that have been attached to this event. For more information on event tagging, see **Policies → Common Objects → Other → Tags**, and [Apply tags to identify and group events](apply-tags-identify-group-events.mdx).

### Filter the list to search for an event {#filter-the-list-to-search-for-an-event}
...
![](/images/2016-07-08_000133_DS10=3cd4b1ae-2323-4b3e-8ef0-b7ca27506185.webp)
...
### Improve logging performance {#improve-logging-performance}
...
- [Events in JSON format](events-json-format.mdx)
- [Apply tags to identify and group events](apply-tags-identify-group-events.mdx)
- [Rank events to quantify their importance](rank-events-importance.mdx)
- [Reduce the number of logged events](reduce-number-logged-events.mdx)
- [Set up Amazon SNS](set-up-amazon-sns.mdx)
- [Log and event storage](log-event-storage.mdx)
- [Forward Events to a Syslog or SIEM Server](forward-events-syslog-siem-server.mdx)
- [System events](system-events.mdx)
- [Activity Monitoring events](activity-monitoring-events.mdx)
- [Anti-Malware events](anti-malware-events.mdx)
- [Web Reputation events](web-reputation-events.mdx)
- [Device Control events](device-control-events.mdx)
- [Application Control events](application-control-events.mdx)
