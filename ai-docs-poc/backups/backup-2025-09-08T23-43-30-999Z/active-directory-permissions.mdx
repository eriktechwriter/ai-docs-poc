...
ai_context:
  source_file: active-directory-permissions.md
  conversion_date: '2025-09-08T23:42:26.107Z'
...
The following table outlines the permission scope options available when [configuring the Active Directory (on-premises) connector](adfs-user-access-control.mdx) in Third-Party Integration.
...
To enable user access control [response actions](response-actions-intro.mdx) on connected Active Directory accounts, you must [configure the Active Directory server connection](adfs-user-access-control.mdx) using a service account with sufficient permissions. You can grant the necessary permissions using one of the following three options.
