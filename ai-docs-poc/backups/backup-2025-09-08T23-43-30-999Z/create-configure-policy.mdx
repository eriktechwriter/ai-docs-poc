---
slug: trend-vision-one-create-configure-policy
title: Create and configure a policy
last_updated: '2025-09-08'
ai_context:
  source_file: create-configure-policy.md
  conversion_date: '2025-09-08T23:42:21.476Z'
conversion_status: converted
translation_ready: true
---

:::::::::::: {role="main"}
Use policies to protect computers using one or more Server & Workload Protection modules.

:::tip

For information about configuring specific protection modules for a policy, see the following guides:
...
- [Configure Application Control](config-app-control-policy.mdx)
...
Before you use the API, you should understand the essential concepts about policies. For background information, see [Create policies to protect your computers and other resources](create-policies.mdx).
...
To see the available policy properties, expand the 200 response to the [Describe a Policy](https://cloudone.trendmicro.com/docs/workload-security/api-reference/tag/Policies) operation in the API Reference.
:::
...
[View source](https://github.com/deep-security/automation-center-sdk-samples/blob/master/python/src/policy_examples.py)
...
# Add the new policy to Server & Workload Protection
...
To use the API to interact with policies, use the `/api/policies` endpoint. (See the [Policies](https://cloudone.trendmicro.com/docs/workload-security/api-reference/tag/Policies) group of operations in the API Reference.)

For information about searching, see [Search for Resources](search-for-resources.mdx). For information about authenticating API calls, see [Authenticate with Server & Workload Protection](send-your-first-request-using-api.mdx).
...
- Use a `ComputersApi` object to update the computer on Server & Workload Protection.
...
You can override a policy at the computer level. See [Configure Computers to Override Policies](override-policies-computer.mdx).
:::

## Configure policy and default policy settings {#configure-policy-and-default-policy-settings}

Policy Settings control many of the behaviors of the protection modules and the Server & Workload Protection platform. Therefore, many tasks that you automate using the API require you to configure policy settings.

For a list of policy and default policy settings, see [Default policy, policy, and computer settings](settings-reference.mdx) in the Settings Reference.

## Default setting values and overrides {#default-setting-values-and-overrides}

Server & Workload Protection policies are hierarchical. A policy's location in the hierarchy determines the default values of their settings:
...
For more information about the policy hierarchy and inheritance, see [Policies, inheritance, and overrides](policies-inheritance-overrides.mdx).

## Policy setting and default policy setting classes {#policy-setting-and-default-policy-setting-classes}

The Server & Workload Protection SDKs provide the following classes for storing policy and default policy settings. These classes are used to pass setting values between Server & Workload Protection and SDK or API clients.
...
The settings of the `DefaultPolicySettings` and `PolicySettings` classes are identical, with a few exceptions. (See [Default policy, policy, and computer settings](settings-reference.mdx) in the Settings Reference.)

## Retrieve the value of a policy setting or default policy setting {#retrieve-the-value-of-a-policy-setting-or-default-policy-setting}
...
When you retrieve a setting value, identify the setting by name. The value is returned as a `SettingValue` object. For a list of policy and default policy settings, see [Default policy, policy, and computer settings](settings-reference.mdx) in the Settings Reference.

The following example retrieves the firewall network engine mode of a policy.

[View source](https://github.com/deep-security/automation-center-sdk-samples/blob/master/python/src/settings_examples.py)

``` codeblock
# Get the policy details from Server & Workload Protection
policies_api = api.PoliciesApi(api.ApiClient(configuration))
...
## List all policy or default policy settings {#list-all-policy-or-default-policy-settings}

You can retrieve all policy and default policy settings in a single call. The way you retrieve settings from Server & Workload Protection depends on the setting class.

- *Default policy settings:* Use a `PoliciesApi` object to get a `DefaultPolicySettings` object from the manager.

- *Policy settings:* Use a `PoliciesApi` object to get a policy from Server & Workload Protection as a `Policy` object. Then, get the `PolicySettings` object from the `Policy` object.

For examples, see the [List default policy settings](https://cloudone.trendmicro.com/docs/workload-security/api-reference/tag/Policies) and [Describe a policy](https://cloudone.trendmicro.com/docs/workload-security/api-reference/tag/Policies) operations in the Policies section of the API Reference.

## Configure a single policy or default policy setting {#configure-a-single-policy-or-default-policy-setting}

The `PoliciesApi` class enables you to set the value of a single setting for a policy or for the default policy settings.

### Procedure {#procedure}

1.  Create a `SettingValue` object and set the value (all values are strings). When settings accept one value from a list of choices, you can either use the ID of the choice or the exact wording of the choice as it appears in the Server & Workload Protection console.
...
# Modify the setting on Server & Workload Protection
policies_api = api.PoliciesApi(api.ApiClient(configuration))
...
Also see the [Modify a Policy Setting](https://cloudone.trendmicro.com/docs/workload-security/api-reference/tag/Policies) operation in the API Reference.

## Configure multiple policy and default policy settings {#configure-multiple-policy-and-default-policy-settings}
...
For either class of settings, the way you modify the setting on Server & Workload Protection is slightly different:
...
Settings in the `DefaultPolicySettings` or `PolicySettings` object that have no values are unchanged on Server & Workload Protection.

Server & Workload Protection validates all modified settings before persisting the values. If one or more settings in the object is invalid, none of the modified settings are persisted. The error response includes the reason for each failure.

:::note

When Server & Workload Protection validates setting values, it does not check that interdependent settings are concordant.
:::
...
For an example of configuring a system setting, see [Configure Server & Workload Protection System Settings](configure-system-settings.mdx).

## Reset policy overrides {#reset-policy-overrides}
...
## Reset an ID reference {#reset-an-id-reference}
...
For example, the [Set the real time Anti-Malware scan configuration](configure-anti-malware-sdk.mdx) example overrides the realTimeScanConfigurationID for a policy. To reset the property to inherit the value from the parent policy, set `realTimeScanConfigurationID` to `0`.

## Reset a setting {#reset-a-setting}

The `PoliciesApi` class provides methods for resetting a single policy setting. For an example, see the Reset the Value of a Policy Setting operation in the Policies section of the [API Reference](https://cloudone.trendmicro.com/docs/workload-security/api-reference/).

To reset several policy settings at the same time, use a `PolicySettings` object with `PoliciesApi` to set the value of the settings to an empty string. For example, the [Configure Firewall](configure-firewall-sdk.mdx) example overrides the value of the `FirewallSettingReconnaissanceEnabled` setting for a policy. To reset the setting to inherit the value from the parent policy, you set the value to `""`.

## Reset the status of a security module {#reset-the-status-of-a-security-module}

To reset the status of a security module, modify the policy and set the value of the module's state to `inherited`.

For example, the [Turn on Application Control](config-app-control-policy.mdx) example overrides the status property of the Application Control module to `on` for a policy. To reset the setting to inherit the value from the parent policy, you set the status value to `inherited`.

## Reset a rule {#reset-a-rule}
...
## Reset all overrides of a rule {#reset-all-overrides-of-a-rule}
...
To see a code example, go to the Reset firewall rule overrides operation of Policy Firewall Rule Details in the [API Reference](https://cloudone.trendmicro.com/docs/workload-security/api-reference/), and see the code example for the operation.

## Selectively reset overrides of a rule {#selectively-reset-overrides-of-a-rule}

Use the following procedure to reset only some properties of a rule.

### Procedure {#procedure-2}

1.  Obtain all of the overrides for the policy's rule. Use the `describe` method of a module-specific rules details class with the `overrides` parameter set to `true`. Save the results. (See [About the Overrides Parameter](overrides-parameter.mdx).)
...
# Modify the rule on Server & Workload Protection
