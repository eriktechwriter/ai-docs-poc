---
slug: trend-vision-one-connecting-microsoft-aks-clusters
title: Connect Microsoft AKS clusters
last_updated: '2025-09-08'
ai_context:
  source_file: connecting-microsoft-aks-clusters.md
  conversion_date: '2025-09-08T23:42:21.893Z'
conversion_status: converted
translation_ready: true
---

:::: {role="main"}
Container Security supports protection on connected Microsoft Azure AKS containers.
...
- For users that want to use the [Isolate Container task](isolate-container-task.mdx), you must ensure that your cluster implements the **Calico** network policy.
:::
...
6.  If you want Container Security to send data to Cloud Risk Management and receive [CREM Risk Insights](executive-dashboard.mdx), select **Map to cloud account**.
    ...
    1.  In the dropdown menu, select **Azure**.

    2.  In another browser tab, sign in to the Azure account that hosts the cluster.

    3.  Go to the **Kubernetes services**.

    4.  Click the cluster name you want to protect with Container Security.

    5.  Click **Properties** and copy the **Resource ID** value for the cluster.

    6.  Back in the Container Security Protect Cluster screen, paste the value in the **Resource ID** field.
...
8.  To ensure that Container Security does not impact any of the following Kubernetes management systems, select the systems in the **Namespace Exclusions** dropdown menu.
    ...
    Options with an asterisk (\*) represent sets of related namespaces. For details, see [Grouped namespaces](grouped-namespaces.mdx).
    :::
...
12. For users deploying Container Security protection the cluster for the first time:
    ...
    3.  Copy the entire `helm install` script in the second input field and execute the Helm script on your cluster.
