---
slug: trend-vision-one-cis-aks17-423
title: >-
  4.2.3 - Minimize the admission of containers wishing to share the host IPC
  namespace (Automated)
last_updated: '2025-09-08'
ai_context:
  source_file: cis-aks17-423.md
  conversion_date: '2025-09-08T23:42:23.788Z'
...
Search for the hostIPC Flag:

In the YAML output, look for the `hostIPC` setting under the spec section to check if it is set to `true`.
...
``` codeblock
kubectl get pods --all-namespaces -o json | jq -r '.items[] | 
select(.spec.hostIPC == true) | select(.metadata.namespace != 
