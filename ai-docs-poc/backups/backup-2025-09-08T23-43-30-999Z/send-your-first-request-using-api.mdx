---
slug: trend-vision-one-send-your-first-request-using-api
title: Send your first request using the API
last_updated: '2025-09-08'
ai_context:
  source_file: send-your-first-request-using-api.md
  conversion_date: '2025-09-08T23:42:12.920Z'
...
- An SDK client library, if you choose to use one. Go to the [Python SDK page](python-sdk.mdx) to download the client library and learn how to add it to your development environment.
- The runtime environment for the programming language of your client library.

:::tip

To start exploring the API right away, instead of using a client library you can use an HTTP client such as [Postman](https://www.getpostman.com/), [Paw](https://paw.cloud/), or [curl](https://curl.haxx.se/).
:::

### Authenticate with Server & Workload Protection {#authenticate-with-server-workload-protection}

Server & Workload Protection uses API keys for authenticating HTTP requests.

The way you authenticate HTTP requests depends on whether you're using a new Trend Micro Cloud One account or an older legacy account. For information on the two types of accounts, see [Changes to Trend Micro Cloud One accounts](https://docs.trendmicro.com/en-us/documentation/article/trend-micro-cloud-one-identity-account-management-c1-accounts-about).
...
### Authenticate with a Trend Micro Cloud One API key {#authenticate-with-a-trend-micro-cloud-one-api-key}
...
### Authenticate with a legacy API key {#authenticate-with-a-legacy-api-key}
...
### Perform a GET request: list policies {#perform-a-get-request-list-policies}

To start exploring the API, go to the [List Policies](https://automation.trendmicro.com/cloudone/workload-security/) operation in the Policies section of the API reference. Notice that `List Policies` is a GET request on the `policies` endpoint:

![](/images/apiref_getpolicies=5b5e8bec-8e74-4ff9-bbb1-900f481afec9.webp)

### Use an HTTP client {#use-an-http-client}
...
### Use a client library {#use-a-client-library}
...
### Perform a POST request: search firewall rules {#perform-a-post-request-search-firewall-rules}

Perform a POST request to search for firewall rules. In the API reference, the [Search Firewall Rules](https://automation.trendmicro.com/cloudone/workload-security/) operation (Firewall Rules section) for the `firewallrules` endoint is a POST request to the path `firewallrules/search`.

![](/images/apiref_fwrules=827c9ed7-74fc-4c90-989f-95de5c5926b4.webp)
...
### Use an HTTP client to post {#use-an-http-client-to-post}
...
### Use a client library to post {#use-a-client-library-to-post}
...
### Get the Server & Workload Protection version {#get-the-server-workload-protection-version}
