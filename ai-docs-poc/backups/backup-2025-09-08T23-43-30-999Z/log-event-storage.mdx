---
slug: trend-vision-one-log-event-storage
title: Log and event storage
last_updated: '2025-09-08'
ai_context:
  source_file: log-event-storage.md
  conversion_date: '2025-09-08T23:42:16.458Z'
conversion_status: converted
translation_ready: true
---

::: {role="main"}
See [How long are events stored?](event-logging.mdx) for details on the storage period for system and security events.

Steps to consider to control storage:

### Procedure {#procedure}

1.  Forward security events to external storage. See [Forward events to an external Syslog or SIEM server](forward-syslog-siem-server.mdx).

2.  Set thresholds in the log inspection module for event storage or event forwarding. *Severity clipping* allows you to send events to a Syslog server (if enabled) or to store events based on the severity level of the log inspection rule. See [Thresholds for Event Storage or Event Forwarding](set-up-log-inspection.mdx).

### Next steps {#next-steps}

## Limit log file sizes {#limit-log-file-sizes}
...
3.  Configure these properties:
    ...
    - *Do Not Record Events with Source IP of:* This option is useful if you don't want Server & Workload Protection to make record events for traffic from certain trusted computers.
    ...
    Regardless of the above settings, the cache is flushed whenever events are sent to Server & Workload Protection.
    :::
...
## Event logging tips {#event-logging-tips}
